...
import { Calendar, BookOpen, ClipboardCheck, GraduationCap } from "lucide-react";
    ...
    const fetchFacultyData = async () => {
        try {
            ...
            if (profileRes.ok) {
                ...
                const examsRes = await authFetch(`${API_BASE_URL}/faculty/${profile._id}/exams`);
                if (examsRes.ok) {
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <Card>
                    ...
                    <CardContent>
                        ...
                        <p className="text-xs text-muted-foreground">Periods assigned today</p>
                    </CardContent>
                </Card>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{facultyProfile.department}</div>
                        <p className="text-xs text-muted-foreground">{facultyProfile.role}</p>
                    </CardContent>
                </Card>
            </div>
                ...
                <CardContent>
                    {todaysPeriods.length === 0 ? (
                        ...
                        <div className="space-y-3">
                            {todaysPeriods.sort((a, b) => a.period - b.period).map((period) => (
                                <div key={period._id} className="flex items-center justify-between p-3 border rounded-lg">
                                    ...
                                    <Button size="sm" onClick={() => window.location.href = `/attendance?class=${period.className}&period=${period.period}`}>
                                        Mark Attendance
                                    </Button>
                                </div>
                            ))}
            ...
            <Card>
                ...
                <CardContent>
                    {assignedExams.length === 0 ? (
                        <p className="text-muted-foreground text-center py-4">No exams assigned for grading</p>
                    ) : (
                        <div className="space-y-3">
                            {assignedExams.map((exam) => (
                                <div key={exam._id} className="flex items-center justify-between p-3 border rounded-lg">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <Button size="sm" onClick={() => window.location.href = `/exams/${exam._id}/results`}>
                                            Grade Students
                                        </Button>
                                    </div>
                                </div>
                                ...
                                <div key={day} className="border-l-4 border-primary pl-4">
                                    ...
                                    <div className="grid gap-2 md:grid-cols-2 lg:grid-cols-3">
                                        {dayPeriods.sort((a, b) => a.period - b.period).map((period) => (
                                            <div key={period._id} className="p-2 bg-muted rounded text-sm">
                                                <p className="font-medium">Period {period.period} - {period.subject}</p>
                                                <p className="text-muted-foreground">{period.className}</p>
                                            </div>
                                        ))}
                                    </div>
                                </div>
