...
const FeesRegistration = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const feesRes = await authFetch(`${API_BASE_URL}/fee-structures`);
        if (feesRes.ok) {
    ...
    if (selectedGrade) {
      const filtered = students.filter(
        (s) =>
          ...
          (selectedGrade || "").toLowerCase().trim(),
      );
  ...
  const handleSaveFeeStructure = async () => {
    ...
    try {
      const response = await authFetch(`${API_BASE_URL}/fee-structures`, {
        method: "POST",
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Register fee structures for students
          </p>
        </div>
      </div>
        ...
        <div className="lg:col-span-8">
          {selectedStudent ? (
            <Card>
              <CardHeader>
                <CardTitle>
                  Fee Registration for {selectedStudent.name}
                </CardTitle>
